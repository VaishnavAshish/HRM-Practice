...
passport.use('user', new LocalStrategy({ usernameField: 'email',passReqToCallback: true}, (req, email, password, done) => {
  ...
  pool.connect((err, client, poolDone) => {
    client.query("select * from company INNER JOIN (SELECT company_id,stripe_customer_id,stripe_subscription_id,quickbook_enabled FROM setting) setting ON company.id = setting.company_id AND company.domain=$1 AND company.archived=$2", [domain,false], function(err, company) {
  ...
  if (!req.isAuthenticated()) {
    ...
    //
        pool.connect((err, client, done) => {
          ...
          client.query("select * from company INNER JOIN (SELECT company_id,stripe_customer_id,stripe_subscription_id,quickbook_enabled FROM setting) setting ON company.id = setting.company_id AND company.id=$1 AND company.archived=$2", [req.user.company_id,false], function(err, company) {
            if (err) {
