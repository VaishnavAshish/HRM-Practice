    ...
    pool.connect((err, client, done) => {
        ...
        if (req.body.searchText != '') {
                    ...
                    if (accounts.rows.length > 0) {
                        ...
                        // accounts.rows.forEach(function(account, index) {
                            ...
                            // createAccountArray(accounts.rows,accountIdArr,accountId=>{
                                ...
                                client.query('SELECT id,name,account_id,archived FROM PROJECT WHERE company_id=$1 AND account_id = ANY($2::bigint[]) AND isGlobal=$3 AND id in (SELECT project_id FROM PROJECT_ASSIGNMENT WHERE company_id=$1 AND user_id=$4)', [req.user.company_id, accountId,false,req.body.user_id], function(err, project) {
                                  if (err) {
                                    handleResponse.shouldAbort(err, client, done);
            ...
            if (req.body.status) {
                ...
                queryToExec = 'SELECT e.id ,e.tax ,e.tax_amount ,e.note ,e.status ,e.category ,e.amount ,e.billable ,e.archived ,e.created_date at time zone \''+companyDefaultTimezone+'\' as created_date ,e.modified_date at time zone \''+companyDefaultTimezone+'\' as modified_date ,e.company_id ,e.account_id ,e.project_id ,e.expense_date at time zone \''+companyDefaultTimezone+'\' as expense_date ,e.currency ,e.invoiced ,e.invoice_id,e.submitted ,e.total_amount ,e.user_id ,e.record_id,(' + innerQuery + ') as searchCount FROM EXPENSE e '+whereClause+'AND status=$5';
                searchFieldVal.push(req.body.status);
            } else {
                queryToExec = 'SELECT e.id ,e.tax ,e.tax_amount ,e.note ,e.status ,e.category ,e.amount ,e.billable ,e.archived ,e.created_date at time zone \''+companyDefaultTimezone+'\' as created_date ,e.modified_date at time zone \''+companyDefaultTimezone+'\' as modified_date ,e.company_id ,e.account_id ,e.project_id ,e.expense_date at time zone \''+companyDefaultTimezone+'\' as expense_date ,e.currency ,e.invoiced ,e.invoice_id ,e.submitted,e.total_amount ,e.user_id ,e.record_id,(' + innerQuery + ') as searchCount FROM EXPENSE e '+whereClause;
            }
