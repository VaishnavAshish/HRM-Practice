...
$.fn.kwLookup = function(option){

        $(this).keyup(function(event){
            var look, lookTxt;
            lookupInput = $(this);
            var lookParent  = $(this).closest('.slds-combobox');
            var currentLi = option.list_outer + ' .kwlookup-li-infocus'
            var dropdwon = $(option.list_outer).parent();
            var dropH = $(option.list_outer).parent().height();
            var load  = $('.slds-spinner',lookParent);
            $(option.list_outer).parent().scrollTop(0)
            switch (event.keyCode) {
                case 40:
                    if($(currentLi).index() != $('li:last-child', option.list_outer).index()){
                        $(currentLi).add($(currentLi).next())
                        .toggleClass('kwlookup-li-infocus');
                        dropdwon.scrollTop($('.kwlookup-li-infocus').offset().top-dropH);
                    }
                break;

                case 38:
                    if($(currentLi).index() > 0){
                        $(currentLi).add($(currentLi).prev())
                        .toggleClass('kwlookup-li-infocus');
                        dropdwon.scrollTop($('.kwlookup-li-infocus').offset().top-dropH);
                    }
                break;
                ...
                look = setTimeout(function(){
                    var term = lookupInput.val().trim();
                    if (lookTxt == term || term == "") return;
                    lookTxt = term;
                    if(!lookParent.hasClass('slds-is-open')){
                    function lookupSelected(event){
                        var item = $(event.target).closest('li');
                        var selectedItem = $('.slds-listbox__option .look-text', item).text();
                        var selectedImg = $('.slds-listbox__option img', item).attr('src');
                        var selectedID = $(item).attr('item-id');
                        var Same = false;
                        if(option.searchID){
                            $('li', option.pillContainer).each(function(){
                        ...
                        $(lookParent).removeClass('slds-is-open');
                            if (option.multiPill && !Same){
                                if (option.listImg){
                                    $(option.pillTemplate
                                        .replace('listID', selectedID)
                                        .replace('listitem', selectedItem)
                                        .replace('itemImg', selectedImg)
                                    )
                                    .appendTo(option.pillContainer);
                                }
                                else{
                                    $(option.pillTemplate.replace('listitem', selectedItem))
                                    .appendTo(option.pillContainer);
                                }
                                $('[lookUp-action="close"]').click(multiPillClose);
                                lookupInput.val('').focus();
                            }
                            else if(!option.multiPill) {
                                    if (option.listImg){
                                        $(option.pillTemplate.replace('listitem', selectedItem).replace('itemImg', selectedImg))
                                        .appendTo(lookParent);
                                    }
                                    else{
                                        $(option.pillTemplate.replace('listitem', selectedItem))
                                    ...
                                    lookupInput.closest('.slds-combobox__form-element').addClass('hide');
                                    $('[lookUp-action="close"]').click(pillClose);
                                    lookupInput.closest('.slds-combobox_container').addClass('slds-has-selection');
                                }
                            $(currentLi).removeClass('kwlookup-li-infocus');
                        }
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            var liItem = eval(option.searchJS)
                            $(option.list_outer).empty();
                            if (option.listImg){
                                $.each(liItem, function(n, listitem){
                                    $(option.list_li_template
                                        .replace('listID', listitem[option.searchID])
                                        .replace('listitem', listitem[option.searchTerm])
                                        .replace('public/img/user-default-image.png', listitem[option.listImg])
                                    )
                                    .appendTo(option.list_outer)
                            ...
                            $('li',option.list_outer).first().addClass('kwlookup-li-infocus');
                            $(lookParent ).addClass('slds-is-open');
                            load.addClass('hide');
                        }
                    });
...
$.fn.setkwLookup = function(option){
    lookupInput = $(this);
    var lookParent  = $(this).closest('.slds-combobox');
    ...
    if (option.multiPill){
        ...
        if (option.imgTerm){
            $.each(option.itemJSON, function(n, item){
                $(option.pillTemplate
                    .replace('listID', item.id)
                    .replace('listitem', item[option.itemTerm])
                    .replace('itemImg', item[option.imgTerm])
                )
                .appendTo(option.pillContainer);
            });
        }
            ...
            $.each(option.itemJSON, function(n, item){
                $(option.pillTemplate
                    .replace('listID', item.id)
                    .replace('listitem', item.first_name)
                )
                .appendTo(option.pillContainer);
            });
        }
        ...
        if (option.itemImg){
            $(option.pillTemplate
                .replace('listitem', option.itemValue)
                .replace('public/img/user-default-image.png', option.itemImg)
            )
            .appendTo(lookParent);
        }
        else{
            $(option.pillTemplate.replace('listitem', option.itemValue))
            .appendTo(lookParent);
        }
        lookupInput.closest('.slds-combobox__form-element').addClass('hide');
        $('[lookUp-action="close"]').click(pillClose);
...
function multiPillClose(event){
    $(event.target).closest('.slds-listbox-item').remove();
}

function pillClose(event){
    var temp = $(event.target).closest('.slds-combobox__form-element').siblings('.hide');
    $(event.target).closest('.slds-combobox__form-element').remove();
    temp.removeClass('hide');
    temp.closest('.slds-combobox_container').removeClass('slds-has-selection');
    $('input', temp).val('').focus();
}
